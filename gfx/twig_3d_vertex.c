...
{
        struct twig_3d_vertex *p        = smalloc(sizeof(struct twig_3d_vertex));
        u8 i;
        for_i(i, 3) {
                p->bid[i]               = bid[i];
        }
...
static inline void __twig_3d_vertex_set_segment(struct twig_3d_vertex *p, struct node_data_segment *seg, void *bytes, u32 len, u8 index)
{
        if(!list_singular(&p->tree_head)) {
                struct list_head *head = p->tree_head.next;
                struct list_head *node_head = (struct list_head *)
