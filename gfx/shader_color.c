...
struct shader *shader_color_alloc()
{
        ...
        /* vertex */
        struct shader_buffer_descriptor *sbd = shader_buffer_descriptor_alloc(3 * sizeof(float), 0);
        ...
        /*z*/
        sbd = shader_buffer_descriptor_alloc(sizeof(float), 1);
        sad = shader_attribute_descriptor_alloc(ATTRIBUTE_FLOAT, 0, "z");
        array_push(sbd->attributes, &sad);
        array_push(des->buffers, &sbd);
        /* transform */
        sbd = shader_buffer_descriptor_alloc(16 * sizeof(float), 1);
        sad = shader_attribute_descriptor_alloc(ATTRIBUTE_MAT4, 0, "transform");
        array_push(sbd->attributes, &sad);
        array_push(des->buffers, &sbd);
        /* color */
        sbd = shader_buffer_descriptor_alloc(4 * sizeof(float), 1);
